...
def get_inception_activations(inps):
    ...
    for i in range(n_batches):
        ...
        act[i * BATCH_SIZE:(i + 1) * BATCH_SIZE] = activations.eval(feed_dict = {inception_images: inp}, session = session)
    return act

def activations2distance(act1, act2):
     return fcd.eval(feed_dict = {activations1: act1, activations2: act2}, session = session)
